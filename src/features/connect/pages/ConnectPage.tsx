...
import { usbService, type UsbDevice } from "../../../core/services/usb/UsbService";
import { ArrowLeft, PlugZap, RefreshCw, Send, Usb, XCircle } from "lucide-react";

interface Props { setPage: (page: Page) => void; backTo?: Page }
...
function ConnectPage({ setPage, backTo }: Props) {
  ...
  useEffect(() => {
    return usbService.subscribe(() => forceUpdate((x) => x + 1));
  }, []);

  const addLog = (msg: string) => usbService.addLog(msg);
  ...
  const scan = async () => {
    ...
    } catch (e) { addLog(`Scan failed: ${String(e)}`); }
  };
  ...
  const connect = async (device: UsbDevice) => {
    try {
      ...
      } catch (e) { addLog(`DTR failed: ${String(e)}`); }

      usbService.startReading();
    } catch (e) { addLog(`Connection failed: ${String(e)}`); }
  };
  ...
  const read = async () => {
    try {
      ...
      if (data) usbService.addTerminal(data);
      addLog(`Read: ${data || "(empty)"}`);
    } catch (e) { addLog(`Read failed: ${String(e)}`); }
  };

  return (
    <div className="min-h-screen bg-background">
      <header className="flex items-center gap-3 p-4 border-b border-border">
        <button onClick={() => setPage(backTo ?? { name: "home" })} className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center">
      ...
      <main className="p-5 max-w-2xl mx-auto flex flex-col gap-4">
        <div className="p-5 rounded-3xl bg-card border-2 border-border text-center">
          ...
          {state.device && (
            <p className="text-xs text-muted-foreground mt-2">
              Device ID: {state.device.deviceId} • VID: {state.device.vendorId} • PID: {state.device.productId}
            </p>
          )}
        </div>

        <button onClick={scan} disabled={state.connecting}
          ...
          <RefreshCw className={`w-5 h-5 ${state.connecting ? "animate-spin" : ""}`} />
          Scan USB Devices
        </button>
        ...
        {devices.length > 0 && (
          <div className="flex flex-col gap-2">
            {devices.map((device) => (
              <button key={device.deviceKey || String(device.deviceId)} onClick={() => connect(device)}
                ...
                <div className="font-bold flex items-center gap-2">
                  <PlugZap className="w-4 h-4" /> {device.deviceName || "USB Device"}
                </div>
                <div className="text-xs text-muted-foreground mt-1">
                  Device ID: {device.deviceId} • VID: {device.vendorId} • PID: {device.productId}
                </div>
                {device.deviceKey && <div className="text-[10px] text-muted-foreground mt-1 break-all">Key: {device.deviceKey}</div>}
              </button>
            ))}
          </div>
        )}
        ...
        {state.connected && (
          <div className="grid grid-cols-3 gap-2">
            <button onClick={sendTest} className="py-3 rounded-2xl bg-secondary font-bold flex items-center justify-center gap-2">
              <Send className="w-4 h-4" /> Send
            </button>
            <button onClick={read} className="py-3 rounded-2xl bg-secondary font-bold">Read</button>
            <button onClick={disconnect} className="py-3 rounded-2xl bg-destructive text-white font-bold flex items-center justify-center gap-2">
        ...
        {state.connected && (
          <div className="flex gap-2">
            <input value={command} onChange={(e) => setCommand(e.target.value)}
              ...
              className="flex-1 px-3 py-3 rounded-xl border-2 border-border bg-card" placeholder="Type command..." />
            <button onClick={sendCommand} className="px-4 py-3 rounded-xl bg-primary text-primary-foreground font-bold">Send</button>
          </div>
        )}
